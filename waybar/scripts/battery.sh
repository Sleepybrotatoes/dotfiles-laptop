...
for bat in /sys/class/power_supply/BAT*; do
    ...
    status_now=$(cat "$bat/status")
    if [ "$status_now" = "Charging" ]; then
        is_charging=true
    fi
    if [ "$status_now" = "Full" ]; then
        is_full=true
    fi

done

# 3. Calculate combined percentage (Safely, with rounding)
if [ "$total_full" -gt 0 ]; then
    capacity=$(( (100 * total_now + total_full / 2) / total_full ))
else
    capacity=0
fi
